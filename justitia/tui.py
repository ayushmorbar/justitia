        ...
        with TabbedContent(initial="generate"):
            with TabPane("Generate Policy", id="generate"):
                yield Container(
                    ...
                    Horizontal(
                        Button(
                            "Generate Policy 🧠", id="generate_btn", variant="primary"
                        ),
            ...
            with TabPane("Test Policy", id="test"):
                yield Container(
                    Static(
                        "Policy testing will be implemented here", id="test_content"
                    ),
                    classes="container",
                )
    ...
    async def generate_policy(self) -> None:
        ...
        if not norms_text:
            output_log.write_line(
                "[red]❌ Please enter policy norms before generating.[/red]"
            )
            return
        ...
        try:
            ...
            pg = PolicyGenerator(
                domain=self.current_domain, reasoning_effort=self.current_effort
            )
            ...
            if policy_json:
                ...
                output_log.write_line(
                    f"[bold cyan]📋 Generated {rules_count} policy rules:[/bold cyan]"
                )

                for i, rule in enumerate(policy_json.get("rules", []), 1):
                    output_log.write_line(
                        f"  {i}. {rule.get('description', 'No description')}"
                    )
                    output_log.write_line(
                        f"     Pattern: [dim]{rule.get('pattern', 'N/A')}[/dim]"
                    )
                    output_log.write_line(
                        f"     Severity: [{'red' if rule.get('severity') == 'high' else 'yellow' if rule.get('severity') == 'medium' else 'green'}]{rule.get('severity', 'unknown')}[/]"
                    )
                    output_log.write_line("")
            ...
            if audit_notebook:
                output_log.write_line(
                    "[bold magenta]🔍 Reasoning Process (Preview):[/bold magenta]"
                )
                preview = (
                    audit_notebook[:300] + "..."
...
Create structured, testable policy rules with clear rationale.""",
        ...
        output_log.write_line(
            f"[green]📝 Loaded sample norms for {self.current_domain}[/green]"
        )
