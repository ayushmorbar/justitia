...
def init(
    domain: str = typer.Argument(
        ..., help="Policy domain (e.g., content-moderation, code-review)"
    ),
    ...
    console.print(
        Panel.fit(
            f"🎯 Initializing JUSTITIA policy project: {domain}", style="bold green"
        )
    )
...
def generate(
    ...
    effort: str = typer.Option(
        "medium", "--effort", "-e", help="Reasoning effort: low/medium/high"
    ),
    ...
    console.print(
        Panel.fit(
            f"🧠 Generating policy with reasoning effort: {effort}", style="bold blue"
        )
    )
    ...
    domain = (
        input_file.parent.name if input_file.parent.name != "." else "default-domain"
    )
    if output:
        output_path = output
    ...
    try:
        ...
        if policy_json:
            ...
            console.print(
                "⚠️ JSON parsing failed; see audit notebook for details", style="yellow"
            )

        console.print("\n[bold cyan]📝 Audit Notebook Preview:[/bold cyan]")
        audit_preview = (
            audit_notebook[:300] + "..."
            if len(audit_notebook) > 300
            else audit_notebook
        )
        console.print(
            audit_preview if audit_preview.strip() else "No reasoning captured"
        )
...
def test(
    ...
    console.print(Panel.fit("🧪 Running policy validation tests", style="bold yellow"))
    ...
    try:
        ...
        # Run tests
        console.print(
            f"🔍 Testing {len(test_suite.test_cases)} cases against {len(test_suite.policy.rules)} rules..."
        )
...
def create_samples(
    ...
    output_dir: Path = typer.Option(
        "./examples", "--output", "-o", help="Output directory"
    ),
):
    """Create sample norms and test cases for a domain"""
    console.print(
        Panel.fit(f"� Creating sample files for domain: {domain}", style="bold cyan")
    )
