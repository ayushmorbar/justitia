...
class TestCase(BaseModel):
    ...
    expected_violations: List[str] = Field(
        ..., description="Rule IDs expected to be triggered"
    )
...
class PolicyTestSuite:
    ...
    def __init__(
        ...
        test_cases_data: Dict[str, Any] = None,
    ):
        if policy_data is not None and test_cases_data is not None:
            ...
            raise ValueError(
                "Either provide file paths (policy_path, cases_path) or data (policy_data, test_cases_data)"
            )
    ...
    def run_tests(
        self, policy_data: Dict[str, Any] = None, test_cases_data: Dict[str, Any] = None
    ):
        """
    ...
    def run_single_test(self, test_case: TestCase) -> TestResult:
        ...
        for rule in self.policy.rules:
            try:
                if re.search(
                    rule.pattern, test_case.text, flags=re.IGNORECASE | re.MULTILINE
                ):
    ...
    def generate_report(self, results: List[TestResult]) -> Dict[str, Any]:
        ...
        average_score = (
            sum(r.score for r in results) / total_tests if total_tests > 0 else 0.0
        )
...
Average Score: {summary['average_score']:.3f}
        ...
        console.print(
            Panel(
                summary_text.strip(),
                title="📊 Test Summary",
                style="green" if summary["pass_rate"] > 0.8 else "yellow",
            )
        )
